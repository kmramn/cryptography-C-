...
int wmain()
{
    ...
    Pp(hr);
    double long i = 0;
    double long B0 = pow((long)2, (long)40);
    for (i = 0; i < B0; i++)
    {
    ...
    for (unsigned int i = 0; i < B; i++)
    {
        ...
        mpz_mul(x1HashTable[i], h, T);
        //Pp(x1HashTable[i]);
        mpz_mod(x1HashTable[i], x1HashTable[i], p);
        //Pp(x1HashTable[i]);
    }
    mpz_clear(x1); /*mpz_clear(r);*/ mpz_clear(T);
    ...
    for (nx0 = 0; nx0 < B; nx0++)
    {
        //mpz_init_set_si(x0, nx0);
        //mpz_powm(r, gB, x0, p[0]);
